...
//  AryaLog
//
//  Expressed breast milk feeding entry form with volume picker
//

import SwiftUI
import CoreData

struct EBMFormView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Amount") {
                    EBMVolumePicker(value: $volumeML)
                }
            ...
            .navigationTitle("Log Expressed Breast Milk")
    ...
    private func saveLog() {
        ...
        log.id = UUID()
        log.activityType = ActivityType.expressedBreastMilk.rawValue
        ...
        do {
            ...
            print("Failed to save EBM log: \(error)")
        }
    }
}

struct EBMVolumePicker: View {
                ...
                ForEach(presets, id: \.self) { preset in
                    Button(action: { value = preset }) {
                        Text("\(preset) mL")
                            ...
                            .padding(.vertical, 12)
                            .background(value == preset ? Color.cyan : Color(.systemGray5))
